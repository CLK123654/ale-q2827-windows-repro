...
function Invoke-AlePrecheck([string]$InputRoot, [string]$ReferenceRoot, [string]$OutputRoot) {
  ...
  & $aleRuby (Join-Path $ReferenceRoot "output/run_precheck.rb")
...
try {
  ...
  $env:KUBECTL_BIN = $aleKubectl
  $env:RUBY_BIN = $aleRuby
  & $aleRuby (Join-Path $negativeRoot "reference/output/run_precheck.rb") 2>&1 | Out-File (Join-Path $aleEvidence "negative.log")
